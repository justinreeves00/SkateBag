    ...
    <div className="min-h-screen text-white selection:bg-teal-500/30">
        ...
        <div className="max-w-6xl mx-auto px-6 py-3 flex justify-between items-center border-b border-white/5">
            ...
            {!isAuthenticated ? (
              ...
              <>
                <div className="flex items-center gap-2 bg-white/5 px-3 py-1.5 rounded-full border border-white/10">
                  ...
                  <span className="text-[10px] text-teal-100 font-bold leading-none truncate max-w-[120px] md:max-w-none uppercase tracking-tighter">{userEmail}</span>
                </div>
                {userEmail === "justinreeves00@gmail.com" && (
                  <a 
                    href="/admin" 
                    className="text-[9px] font-black text-teal-400 uppercase tracking-widest hover:text-white transition-colors bg-teal-500/10 px-3 py-1.5 rounded-full border border-teal-500/20"
                  >
                    Admin
                  </a>
                )}
          ...
          <div>
            {!isAuthenticated ? (
              <a
                href="/login"
                className="inline-block px-5 py-2 rounded-full bg-white text-black text-[10px] font-bold uppercase tracking-widest hover:bg-teal-400 transition-all shadow-lg"
              >
                Sign in
              </a>
            ) : (
            ...
            <div className="flex flex-col lg:flex-row lg:items-center justify-between gap-8">
              <div className="space-y-6">
                <div className="flex items-center gap-5">
                  <SkateBagLogo size={64} className="shadow-2xl shadow-teal-500/20 transform -rotate-3 shrink-0 rounded-2xl overflow-hidden" />
                  <div className="space-y-1">
                    ...
                    <p className="text-[10px] font-bold text-teal-400/60 uppercase tracking-[0.4em] ml-1">
                      What&apos;s in your bag?
                    </p>
                  </div>
                </div>
                ...
                <div className="flex items-center p-1 rounded-2xl bg-white/5 border border-white/5 w-fit">
                  ...
                  <button 
                    ...
                    className={`px-4 py-2.5 rounded-xl text-[10px] font-bold uppercase tracking-widest transition-all ${statusFilter === "landed" ? "bg-teal-500 text-white shadow-lg" : "text-slate-400 hover:text-white"}`}
                  >
                    Landed
                  </button>
                  <button 
                    onClick={() => setStatusFilter("locked")}
                    className={`px-4 py-2.5 rounded-xl text-[10px] font-bold uppercase tracking-widest transition-all ${statusFilter === "locked" ? "bg-teal-800 text-white shadow-lg" : "text-slate-400 hover:text-white"}`}
                  >
                    On Lock
                  </button>
                </div>
                ...
                {isAuthenticated && (
                  <div className="flex gap-10 mb-1 px-2">
                    <div className="flex flex-col">
                      <span className="text-[10px] text-slate-500 uppercase font-bold tracking-[0.2em] mb-2">My Bag</span>
                      <div className="flex items-end gap-3">
                        ...
                        <div className="w-20 h-1 bg-white/5 rounded-full overflow-hidden mb-1.5">
                          <div className="h-full bg-gradient-to-r from-teal-500 to-teal-200 transition-all duration-1000" style={{ width: `${progress}%` }} />
                        </div>
                      </div>
                    </div>
                    <div className="flex flex-col">
                      <span className="text-[10px] text-slate-500 uppercase font-bold tracking-[0.2em] mb-2">Practice</span>
                ...
                <div className="flex-1 w-full space-y-4">
                  <div className="relative group flex items-center gap-3">
                    <div className="relative flex-1">
                      <div className="absolute inset-y-0 left-5 flex items-center pointer-events-none transition-colors group-focus-within:text-teal-400 text-slate-500">
                      ...
                      <input
                        ...
                        className="w-full bg-white/5 border border-white/10 rounded-full pl-14 pr-6 py-5 text-sm font-medium text-white placeholder-slate-600 focus:outline-none focus:ring-4 focus:ring-teal-500/10 focus:border-teal-500/40 transition-all"
                      />
                    </div>
      ...
      <main className="max-w-6xl mx-auto px-6 pt-16 pb-32">
        <div className="mb-12 flex items-center gap-6">
          <h2 className="text-[11px] font-black text-teal-400 uppercase tracking-[0.4em] whitespace-nowrap">
            {statusFilter === "all" ? "Trick Vault" : statusFilter === "landed" ? "In The Bag" : "Practice List"}
          </h2>
          <div className="h-px w-full bg-gradient-to-r from-teal-500/20 to-transparent"></div>
        ...
        {filtered.length === 0 ? (
          <div className="py-40 flex flex-col items-center justify-center aurora-card rounded-[3rem] border-dashed border-white/10">
            <div className="w-20 h-20 rounded-full bg-teal-500/5 flex items-center justify-center mb-8 border border-teal-500/10">
              <svg width="32" height="32" viewBox="0 0 24 24" fill="none" stroke="#10b981" strokeWidth="2" strokeLinecap="round" strokeLinejoin="round">
                <circle cx="12" cy="12" r="10"/><line x1="8" y1="12" x2="16" y2="12"/>
              </svg>
            </div>
            <p className="text-slate-400 text-lg font-medium mb-8">No bangers found here</p>
            <button 
              onClick={() => { setSearch(""); setCategory("all"); setStatusFilter("all"); }}
              className="px-8 py-4 bg-white text-black hover:bg-teal-400 rounded-full text-xs font-black uppercase tracking-widest transition-all shadow-xl"
            >
              Reset Vault
            </button>
          </div>
